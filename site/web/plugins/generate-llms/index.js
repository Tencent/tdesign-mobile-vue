...
/** 列出 demo 的候选路径：demos/<demoName>.vue（含 kebab/camelCase 双写）与 demos/<demoName>/index.vue。 */
function demoCandidates(componentDir, demoName) {
  ...
  return names.flatMap((name) => [
    path.join(componentDir, 'demos', `${name}.vue`),
    path.join(componentDir, 'demos', name, 'index.vue'),
  ]);
}

/**
 * 读取组件目录下的 demo 源码：demos/<demoName>.vue，回退 demos/<demoName>/index.vue。
 * 输出 Vue SFC 代码块。
 */
...
/**
 * 判断是否为 demo 占位符：匹配 demos/<demoName>.vue 或 demos/<demoName>/index.vue。
...
function normalizeDemoPlaceholders(doc, componentDir) {
  return doc.replace(/\{\{\s*([a-zA-Z][a-zA-Z0-9]*)\s*\}\}/g, (match, name) => {
    if (!/[A-Z]/.test(name) || !isDemoSlot(componentDir, name)) return match;
 ...
 * 此处仅负责 vite 构建钩子分发，并按 mobile-vue 仓库约定注入组件文档与 demo 源码读取器。
 */
export default function generateLlmsPlugin() {
  ...
  return {
    ...
    async closeBundle(error) {
      ...
      // common 子仓文档根目录（组件文档为 docs/mobile/api/<slug>.md，含 frontmatter 与 {{ demo }} 占位符）
      ...
      // 组件文档读取器：common 子仓 docs/mobile/api/<slug>.md（frontmatter + 代码演示 + API），
      // 并追加组件目录 <slug>.md 的 API 部分（站点构建时由 :: BASE_DOC :: 注入，此处手动拼接）
      const readComponentDoc = async (componentDir, slug) => {
        const docPath = path.join(docsRoot, 'mobile/api', `${slug}.md`);
        try {
      ...
      await generateLlmsDocs({
        ...
        platform: 'mobile',
        parseComponentDoc,
        siteTitle: 'TDesign Mobile Vue',
        siteDescription: 'TDesign 适配移动端的组件库，适合在 Vue3.x 技术栈项目中使用。',
