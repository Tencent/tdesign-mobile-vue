...
export default defineComponent({
  ...
  setup(props, { expose }) {
    ...
    const formatValidateResult = <T extends Data>(validateResultList: FormItemValidateResult<T>[]) => {
      const result = validateResultList.reduce(
        (r: Record<string, FormItemValidateResult<T>>, err) => Object.assign(r || {}, err),
        {},
      );
    ...
    const getFirstError = (result: Result) => {
      ...
      return result?.[Object.keys(result as ValidateResultList)?.[0]]?.[0]?.message || '';
    };
    // 校验不通过时，滚动到第一个错误表单
